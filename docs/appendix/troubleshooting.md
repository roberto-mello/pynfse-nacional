...
Quando o erro vier da própria biblioteca, use o `error.code` + `error.message`
para diagnosticar:

- `error.code` identifica a categoria do problema de forma estável
- `error.message` mensagem em Português do Brasil
...
certificado de homologação. Às vezes o arquivo está válido mas o cadastro no
ambiente não.
